...
deps:
	...
	# Ensure local Swift package submodules (e.g. AltSign, Zsign, IDeviceKitten) exist before package resolution.
...
$(SCHEMES): prepare_packages
	# Zsign expects <openssl/...> headers; reuse NexAppleSign's vendored XCFramework to avoid a second OpenSSL copy.
	set -e; \
	...
	if [ -z "$$ALT_SIGN_OPENSSL_XCFRAMEWORK" ]; then \
		echo "Expected NexAppleSign OpenSSL.xcframework after package resolution." >&2; \
		exit 1; \
	fi; \
	case "$(PLATFORM)" in \
		iphonesimulator) \
